...
def download_and_extract_binary(binary_info):
    ...
    with zipfile.ZipFile(binary_info['ZipLocation'], 'r') as zip_ref:
        ...
        # Find the first file with the binary name in the path
        extracted_binary_path = next(
            (f for f in zip_file_contents if binary_info['ZipName'] in f), None)

        # If found, extract the binary
        if extracted_binary_path:
            zip_ref.extract(extracted_binary_path,
                            binary_info['UnzipFolderLocation'])
            extracted_binary_path = Path(
                binary_info['UnzipFolderLocation']) / extracted_binary_path
            os.rename(extracted_binary_path, binary_info['BinaryLocation'])
            print(
                f"{binary_info['ZipName']} downloaded and extracted successfully.")
        else:
            print(
                f"Error: {binary_info['ZipName']} not found in the zip file.")
